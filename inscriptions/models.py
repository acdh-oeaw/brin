...
class Inschrift(models.Model):
    gattung = models.ManyToManyField(SkosConcept, blank=True, related_name="gattung",
        ...
        help_text="Klassifikation der Inschrift nach ihrer Gattung (Grab-, Gedächtnis-, Renovierungsinschrift, Baunachricht, Historische Nachricht, Stiftungsinschrift, Gedenkinschrift, Künstlerinschrift, Datierung etc.)."
        )
    traeger = models.ManyToManyField(SkosConcept, blank=True, related_name="traeger",
        verbose_name="Inschriftenträger",
        help_text="Klassifikation des Inschriftenträgers (Grabplatte, Epitaph, Fassade, Türsturz, Altar, Kelch, Tafelmalerei, Medaillon, Grenzstein, Uhr etc.)."
        )
    ...
    farbtraeger = models.CharField(blank=True, max_length=255,
        ...
        help_text="Beschaffenheit des Inschriftenträgers, im Fall von Steinen oder Putz eine annähernde Beschreibung der Beschaffenheit der Oberfläche"
        )
    ...
    erhaltungszustand = models.CharField(blank=True, max_length=255,
        ...
        help_text="Erhaltungszustand der Inschrift und des Inschriftenträgers (sehr gut, gut, befriedigend, schlecht, sehr schlecht)"
        )
    ...
    kennname = models.ManyToManyField(Person, blank=True, related_name="kennname",
        ...
        help_text="Name/n der in der Inschrift genannten Person/en, Personengruppen (z.B. Klarissen, Kapuziner, Domkapitel) und Orte in normalisierter und ggf. ergänzter Schreibung"
        )
    kuenstler = models.ManyToManyField(Person, blank=True, related_name="kuenstler",
        verbose_name="ausführender Künstler",
        help_text="Informationen über den/die ausführenden Künstler, sofern diese/r bekannt ist/sind"
        )
    resch_kopial_signatur = models.CharField(blank=True, max_length=255,
        verbose_name="Signatur bei Resch",
        help_text="Signatur in Joseph Reschs 'Monumenta veteris ecclesie Brixinensis' (1765) bzw. im 'Supplementum ad Monumenta Brixinensia' (1775)."
        )
    resch_kopial_text = models.TextField(blank=True,
        verbose_name="Transkription bei Resch",
        help_text="Transkription durch Joseph Resch in den 'Monumenta veteris ecclesie Brixinensis' (1765) bzw. im 'Supplementum ad Monumenta Brixinensia' (1775)."
        )
    ...
    bemerkungen = models.TextField(blank=True,
        ...
        help_text="Kommentare zu Inhalt, Textverständnis, graphischen Besonderheiten, Sprache, Formular; biographische oder kunsthistorische Hinweise sowie Verweise auf weiterführende Literatur"
        )
    quellen = models.ManyToManyField(Reference, blank=True,
        verbose_name="Quellen und Literatur",
        help_text="Bisherige Nachweise der Inschriften, ggf. mit dem Zusatz ('Abb.''); wichtige Arbeiten, die zum Verständnis des Inschriftenträgers und vor allem zum Inhalt der In­schrift aufklärend und erläuternd beitragen können"
        )
    ...
    schlagworte = models.ManyToManyField(
        ...
        help_text="Die Beschlagwortung stellt eine erhebliche Hilfe für die Suche nach Einträgen in der Datenbank dar. Schlagwörter können sowohl Personen- und Ortsnamen als auch Titel und Ämter (Bischof, Diakon, Schulmeister) oder Sachbegriffe sein (Engel, Fresko, Erker, Wirtshaus etc.)."
        )
    restaurierungsphasen = models.CharField(blank=True, max_length=255,
        verbose_name="Restaurierungsphasen",
        help_text="Beschreibung früherer Restaurierungen, falls vorhanden kann hier auch auf Restaurierungsberichte verwiesen sein."
        )
    allgemeine_beschreibung = models.TextField(blank=True,
        verbose_name="allgemeine Beschreibung",
        help_text="Besonderheiten des Inschriftenträgers nach kunsthistorischen Gesichtspunkten, Blasonierung von Wappen etc. Die Angaben 'links' und 'rechts' beziehen sich auf den Standpunkt des Betrachters, nur im Fall von Wappenbeschreibungen verstehen sich die Angaben den heraldischen Vorgaben entsprechend."
        )
    schriftbeschreibung = models.TextField(blank=True,
        verbose_name="Schriftbeschreibung",
        help_text="Hier ist die Schrift genauer beschrieben, d.h. man geht auf Besonderheiten, Unregelmäßigkeiten, größere u. kleinere Buchstaben usw. ein. Mittels alphabetischem Fußnotensystem wird im Transkriptionsfeld auch auf Varianten der Schrift bzw. Buchstaben aufmerksam gemacht (siehe dort den sogenannten 'Variantenapparat')."
    ...
    schriftklassifikation = models.ManyToManyField(
        ...
        help_text="Art der verwendeten Schrift (Antiqua, Fraktur, Frühhumanistische Kapitalis, Gotico-Antiqua, Gotische Majuskel, Gotische Minuskel, Kapitalis, Romanische Majuskel)."
    ...
    genauer_standort = models.ForeignKey(
        ...
        help_text="Lokalisierung der Inschrift an ihrem Standort zum Zeitpunkt der Aufnahme anhand einer hierarchischen Darstellung (Ort, Fraktion, Weiler/Adresse, genauer Standort). Beispiel: Gemeinde: Brixen, Fraktion: Dombezirk/Altstadt, Weiler/Adresse: Runggadgasse 21, Genauer Standort: Südwand, Erker."
        )
    alter_standort = models.CharField(blank=True, max_length=255, null=True,
        verbose_name="früherer Standort",
        help_text="Frühere Aufstellungs- oder Aufbewahrungsorte, vor allem bei den Brixner Grabmälern mit Bezug auf Joseph Reschs 'Monumenta veteris ecclesie Brixinensis' (1765) und das 'Supplementum ad Monumenta Brixinensia' (1775)."
        )
